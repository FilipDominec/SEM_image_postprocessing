...
## Prepare the logo
def put_image(im, logo_img, x, y, color=1):
...
imname = sys.argv[1] # '/home/dominecf/LIMBA/SEM-dle_cisla_vzorku/2019/319B-FH-190613_190625/10NSA.TIF'
...
size_x = 117500. / float(ih.get('Magnification','nan')) 
...
im = put_image(im, logo_im, x=0, y=int(im.shape[0]-int(ch*4/2)-logo_im.shape[0]/2))
...
im = im_print(im, '{:<6.0f} {:<6.1f} {:<6.2f} {:<6} {:<13}'.format(
    float(ih.get('flAccV','nan')), float(ih.get('flSpot','nan')), float(ih.get('flWD','nan')), 
    '{:<.0f}'.format(float(ih.get('Magnification','nan')))+'x', 
...
im = putscale(im, xpos+465, im.shape[0]-ch*3, ch, scale_bar/size_x*im.shape[1])
