...
DENORM_EXP   =  .0      # partial de-normalization: EDX saves images as normalized. The more 
...
if 'bgim_name' in locals()  and  'lab_name' in locals():
    ...
    shift, _ = pnip.find_affine_and_shift(labim_resc[:,:], bgim[50:-50,50:-50], max_shift=0.15, decim=1, use_affine_transform=False)
    bgim3 = 0 * np.dstack(np.pad(bgim, 50, mode='constant') for ch in range(3))[:,:,:]
    pnip.paste_overlay(bgim3, bgim, shift, np.array([1,1,1])) # , normalize=np.max(newimg_crop)

    imageio.imsave('edx_bgim3.png', bgim3)

    #im_resc = np.dstack([scipy.ndimage.zoom(EDX_coloring[:,:,ch], [bgim.shape[i]/EDX_coloring.shape[i] for i in range(2)], order=1) for ch in range(3)]) #todo use pnip
    #imageio.imsave('edx_raw_remap_resc.png', im_resc)


    EDX_zoomed = np.dstack([scipy.ndimage.zoom(FG_DESATURATE+channel, [bgim.shape[i]/channel.shape[i] for i in range(2)], order=1) for channel in EDX_coloring])
    print("DEBUG: EDX_zoomed = ", EDX_zoomed.shape)
    EDX_padded = np.dstack([np.pad(channel, 50, mode='constant') for channel in EDX_zoomed])
    print("DEBUG: EDX_padded = ", EDX_padded.shape)

    composite = bgim3**BG_GAMMA_CURVE*EDX_padded
