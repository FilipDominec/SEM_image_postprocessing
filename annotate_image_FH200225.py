...
def annotate_initialize():
    logo_im = imageio.imread('logo.png')

    try: 
        typecase_img = imageio.imread('typecase.png')
...
s1=str#=r'C:\Users\Hájek\Documents\Python\Anotation_image_FD' #path to original files to be coverted
s1=input()
s2=str=s1+'\orig' #path to move the original files 

try:
    os.mkdir(s2)
except:
    print('Directory already existing or can not be created')

def annotate_process(imnames):
    for imname in glob.glob(s1+'/*.TIF'):
        ...
        shutil.move(s, s2) # move original files
